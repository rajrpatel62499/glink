...
export class PackageService implements Resolve<any> {
  ...
  onEmployeesChanged: BehaviorSubject<any>; // employee detail selection
  ...
  resolve(
    ...
    if (this.path == 'all') {
      ...
      return new Promise((resolve, reject) => {
        Promise.all([
          this.getAllDataSchedulesByLocationId(),
          this.getAllEmployees()
        ]).then(([schedules, employees]) => {
          this.onParamChanged.next({ title: 'COMMON.PACKAGES' });
          resolve();
        }, reject);
      });
  ...
  private getFilteredSchedules(): Package[] {
  ...
  getAllDataSchedulesByLocationId(): Promise<Package[]> {
    // TODO: KEN
    ...
    filters.dateRangeStart = moment(new Date('1/1/2020')).format('YYYY-MM-DDTHH:mm:ss.ssZ');
    filters.dateRangeEnd = moment(new Date('1/1/2030')).format('YYYY-MM-DDTHH:mm:ss.ssZ');;
          ...
          if (!!this.figmasAll && this.figmasAll.length > 0) {
            ...
            if (!!schedules && schedules.length > 0) {
              const updatedSchedules = schedules.map((item) => {
                // more importantly, to apply UI elements like tags
                const schedItem = new Package(item);
  ...
  deletePackage(schedule, isAll: boolean): any {
    var url = '';
    if (isAll) {
      url = `${this.basePathApi}${Configs.Timesheet}${Configs.Package}${schedule.batchId}`;
    } else {
      url = `${this.basePathApi}${Configs.Timesheet}${Configs.Schedule}${schedule.id}`;
    }
    return new Promise((resolve, reject) => {
      this._httpClient
        .delete(url, {
  ...
  getAllEmployees(): Promise<any> {
    return new Promise((resolve, reject) => {
      ...
      .then((response: any) => {
          ...
          const employees = response.employees.map((item) => {
