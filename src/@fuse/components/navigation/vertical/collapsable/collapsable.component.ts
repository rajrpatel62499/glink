...
export class FuseNavVerticalCollapsableComponent implements OnInit, OnDestroy
{
    ...
    ngOnInit(): void
    {
        ...
        this._router.events
            ...
            .subscribe((event: NavigationEnd) => {
                ...
                {
                    // hotfix:: for url having in collpasable component.
                    if ( !this.isUrlInParent(this.item, event.urlAfterRedirects)) 
                    {
                        this.collapse();
                    }
    ...
    /**
     * Check if the given url can be found
     * in one of the given parent's url
     *
     * @param parent
     * @param url
     * @returns {boolean}
    ...
    isUrlInParent(parent: FuseNavigationItem, url): boolean 
    {
        if (parent?.url?.includes(url)) {
            return true;
        }
